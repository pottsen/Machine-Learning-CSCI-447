...
#2D dictionary
#attribute_prob = {key1:{key1':probability},key2:{key2':probability},...}
#where keyX = the class
# and keyX' = the attribute
#and probability = the probability of this feature(attribute) given the class
#F(keyX'|keyX) = attribute_prob[keyX][keyX']
def classAttributeFrequency(data_set, class_col, class_counts):
    ...
    for i in data_set: # i will represent each record of our data_set
        ...
        for j in range(len(columns)): #j will represent the index of each attribute
            if(j!=class_col): # should not count class occurances given class (==1)
                ...
                try: #if we have seen this attribute before, add 1 to the count
                    att_count = attribute_counts[class_name][attribute_value]
                    attribute_counts[class_name][attribute_value] = att_count + 1
                    #attribute_counts[class_name][attribute_value]+=1 #simplification of last two lines
                except KeyError: #if we have not seen this attribute before, make the count 1
                    try:
                        attribute_counts[class_name].update({attribute_value:1})
                    except KeyError: #if we have not seen this class before initialize the class
                        attribute_counts.update({class_name:{attribute_value:1}})
    ...
    for key_class in attribute_probability:
        for key_attribute in attribute_probability[key_class]:
            attribute_count = attribute_probability[key_class][key_attribute]
            ...
            attribute_probability[key_class][key_attribute] = contidional_probability
